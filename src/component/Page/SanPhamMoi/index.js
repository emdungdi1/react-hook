...
function SanPhamMoi(props) {
		...
		if (props.match.params) {
			function fetchPostsLists() {
				if (props.match.params.SortBy && props.match.params.page) {
					let pageN = 0;
					setActive(`${props.match.params.SortBy}-${props.match.params.GroupBy}`);
	...
	function handlePageChange(pageNumber) {
		if (props.match.params.SortBy && props.match.params.page) {
			...
			history.push(`/SanPhamMoi/SortBy=${'ten_giay'}&&GroupBy=${'desc'}&&Page=${pageNumber}`);
		}
	}
	...
	function sortby(data) {
		if (data === 'ten_giay-asc') {
			...
			history.push(`/SanPhamMoi/SortBy=${'ten_giay'}&&GroupBy=${'asc'}&&Page=${1}`);
		} else if (data === 'ten_giay-desc') {
			setActive('ten_giay-desc');
			history.push(`/SanPhamMoi/SortBy=${'ten_giay'}&&GroupBy=${'desc'}&&Page=${1}`);
		} else if (data === 'date_create-desc') {
			setActive('date_create-desc');
			history.push(`/SanPhamMoi/SortBy=${'date_create'}&&GroupBy=${'desc'}&&Page=${1}`);
		} else if (data === 'gia_ban-asc') {
			setActive('gia_ban-asc');
			history.push(`/SanPhamMoi/SortBy=${'gia_ban'}&&GroupBy=${'asc'}&&Page=${1}`);
		} else if (data === 'gia_ban-desc') {
			setActive('gia_ban-desc');
			history.push(`/SanPhamMoi/SortBy=${'gia_ban'}&&GroupBy=${'desc'}&&Page=${1}`);
		}
	}
	return (
			...
			<div className="container">
				<div className="sort-cate clearfix margin-bottom-10 hidden-xs">
					<div className="sort-cate-left hidden-xs">
						...
						<ul>
							<li
								className={`btn-quick-sort ten_giay-asc`}
								className={isActive === 'ten_giay-asc' ? 'active' : null}
							>
							...
							<li
								className="btn-quick-sort ten_giay-desc"
								className={isActive === 'ten_giay-desc' ? 'active' : null}
							>
							...
							<li
								className="btn-quick-sort gia_ban-asc"
								className={isActive === 'gia_ban-asc' ? 'active' : null}
							>
								<a onClick={() => sortby('gia_ban-asc')} title="Giá thấp đến cao">
									<i></i>Giá thấp đến cao
								</a>
							</li>
							<li
								className="btn-quick-sort gia_ban-desc"
								className={isActive === 'gia_ban-desc' ? 'active' : null}
							>
								<a onClick={() => sortby('gia_ban-desc')} title="Giá cao xuống thấp">
